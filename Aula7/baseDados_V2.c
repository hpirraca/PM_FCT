...
void verifica_numeros(int min, int max, int *num)
{
	do
	{
		scanf(" %d", &(*num));
...
void verifica_caracteres(char *ch)
{
    char palavra[100];
    int i=0;    
...
{
	printf("Introduza o nome (seguido de ponto final): ");
	...
	printf("Introduza a cor dos olhos (seguido de ponto final): ");
	verifica_caracteres(PESSOA[i].infp.olhos);
	printf("Introduza a cor do cabelo (seguido de ponto final): ");
...
{
	printf("Introduza a morada(rua, praca, etc)(seguido de ponto): ");
	...
	printf("Introduza a localidade (seguido de ponto): ");
	verifica_caracteres(PESSOA[i].address.cod_postal3);
}

void le_confidencial(BASEDADOS PESSOA[], int i)
{
	printf("Introduza o partido politico(se nao tiver coloque X)(seguido de ponto): ");
	verifica_caracteres(PESSOA[i].clubes.clube);
	printf("Introduza o clube(se nao tiver coloque X)(seguido de ponto): ");
...
void criar_registo(BASEDADOS PESSOA[], int j)
{
	...
	for(i=0; i<=j; i++)
	{
		if(PESSOA[i].nss.num_serie == 0)
			break;
	}
	
	if(PESSOA[i].nss.num_serie == 0 || j<50)
			le_cidadao(PESSOA, i);
	else
			printf("Não pode introduzir mais registos - Atingiu o limite da base de dados\n\n");
...
{		
	...
	do
	{
	printf("Quantos registos quer criar (numero até 50 - inclusive): ");
	scanf(" %d", &j);
	}while(j>50);
	...
	scanf(" %c", &ch);	
		...
		case 'd': criar_registo(PESSOA, i); if(i<50)i++; break;
